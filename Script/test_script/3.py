...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                               ...
                               QFrame, QStyle, QMenu, QStyleOptionSlider, QCheckBox)
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            value = QStyle.sliderValueFromPosition(
                ...
                pos.x() - rect.left(), rect.right() - rect.left(),
                opt.upsideDown
            )
...
class VideoPlayer(QWidget):
    ...
    def setup_ui(self):
        ...
        ocr_group.setStyleSheet(self.get_groupbox_style())
        ocr_layout = QVBoxLayout(ocr_group)

        self.ocr_list = QListWidget()
        self.ocr_list.setStyleSheet(self.get_list_style())
        self.ocr_list.setMinimumHeight(150)
        ocr_layout.addWidget(self.ocr_list)

        # OCR操作按钮
        ocr_btn_layout = QHBoxLayout()
        self.start_ocr_btn = QPushButton("开始识别")
        self.start_ocr_btn.setIcon(self.style().standardIcon(QStyle.SP_FileDialogContentsView))
        self.start_ocr_btn.clicked.connect(self.start_ocr)
        self.start_ocr_btn.setMinimumHeight(30)
        self.start_ocr_btn.setStyleSheet(self.get_button_style())
        ocr_btn_layout.addWidget(self.start_ocr_btn)

        self.clear_ocr_btn = QPushButton("清除结果")
        self.clear_ocr_btn.setIcon(self.style().standardIcon(QStyle.SP_DialogResetButton))
        self.clear_ocr_btn.clicked.connect(self.clear_ocr_results)
        self.clear_ocr_btn.setMinimumHeight(30)
        self.clear_ocr_btn.setStyleSheet(self.get_button_style())
        ocr_btn_layout.addWidget(self.clear_ocr_btn)

        ocr_layout.addLayout(ocr_btn_layout)
        right_layout.addWidget(ocr_group, 1)
    ...
    def open_video_file(self, file_path):
        ...
        # 设置覆盖层的视频原始尺寸
        self.overlay.set_video_size(QSize(self.original_width, self.original_height))
        # 关闭当前视频
    ...
    def update_position(self):
        ...
        if duration > 0:
            self.current_frame = int(position / duration * self.total_frames)
    ...
    def set_position(self, position):
        ...
        duration = self.media_player.duration()
        if duration > 0:
            new_position = int(position * duration / 100)
    ...
    def prev_frame(self):
        ...
        frame_duration = 1000 / self.fps if self.fps > 0 else 40
        new_position = max(0, current_position - frame_duration)
        self.media_player.setPosition(int(new_position))
        self.status_label.setText(f"上一帧: {int(new_position / 1000)}s")
    ...
    def next_frame(self):
        ...
        frame_duration = 1000 / self.fps if self.fps > 0 else 40
        duration = self.media_player.duration()
        new_position = min(duration, current_position + frame_duration)
        self.media_player.setPosition(int(new_position))
        self.status_label.setText(f"下一帧: {int(new_position / 1000)}s")
    ...
    def mousePressEvent(self, event):
        ...
        if self.mark_btn.isChecked() and event.button() == Qt.LeftButton:
            ...
            self.current_region = [event.pos().x(), event.pos().y(), 0, 0]
            self.overlay.set_current_region(self.current_region)

    def mouseMoveEvent(self, event):
        """鼠标移动事件"""
        if self.drawing:
            pos = event.position().toPoint()  # 使用toPoint()
            # 更新当前区域
    ...
    def update_tag_list(self):
        ...
        for i, region in enumerate(self.regions):
            ...
            item = QListWidgetItem(f"{label_type} [{region_id}] - ({x}, {y}, {w}, {h})")
    ...
    def save_regions(self):
        ...
        if self.original_width <= 0 or self.original_height <= 0:
            QMessageBox.warning(self, "错误", "无法获取视频原始尺寸")
            return []
        ...
        for region in self.regions:
            ...
            # 计算归一化值（基于原始视频尺寸）
            cx = (x + w / 2) / self.original_width
            cy = (y + h / 2) / self.original_height
            nw = w / self.original_width
            nh = h / self.original_height
    ...
    def start_ocr(self):
        ...
        if not self.regions:
            QMessageBox.warning(self, "警告", "请先标记识别区域")
            return
        ...
        # 模拟OCR识别过程
        QTimer.singleShot(2000, self.simulate_ocr_results)

    def simulate_ocr_results(self):
        """模拟OCR识别结果"""
        self.ocr_list.addItem("帧 120: 使用M416淘汰了玩家A")
        self.ocr_list.addItem("帧 245: 使用手雷淘汰了玩家B")
        self.ocr_list.addItem("帧 367: 爆头淘汰了玩家C")
        self.ocr_list.addItem("帧 512: 使用AWM淘汰了玩家D")
        self.status_label.setText("OCR识别完成 (模拟数据)")

    def clear_ocr_results(self):
        """清除OCR识别结果"""
        self.ocr_list.clear()
        self.status_label.setText("OCR结果已清除")
...
class OverlayWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.selected_region_idx = -1
        self.label_colors = {}
        self.video_size = QSize()  # 添加视频原始尺寸属性
        self.widget_rect = QRect()  # 添加控件显示区域属性
    ...
    def resizeEvent(self, event):
        ...
        self.calculate_video_rect()

    def calculate_video_rect(self):
        """计算视频在控件中的实际显示区域"""
        self.widget_rect = self.rect()
        if self.video_size.isValid():
            video_rect = QRect(0, 0, self.video_size.width(), self.video_size.height())
            video_rect = video_rect.scaled(self.widget_rect.size(), Qt.KeepAspectRatio)
            video_rect.moveCenter(self.widget_rect.center())
            self.video_rect = video_rect
        else:
            self.video_rect = self.widget_rect

    def paintEvent(self, event):
        if not self.video_size.isValid():
            return
        ...
        # 计算缩放比例
        scale_x = self.video_rect.width() / self.video_size.width()
        scale_y = self.video_rect.height() / self.video_size.height()

        # 绘制所有标记区域
        for i, region in enumerate(self.regions):
            x, y, w, h, label_type, _ = region

            # 将原始坐标转换为当前显示坐标
            scaled_x = self.video_rect.left() + x * scale_x
            scaled_y = self.video_rect.top() + y * scale_y
            scaled_w = w * scale_x
            scaled_h = h * scale_y

            color = self.label_colors.get(label_type, Qt.yellow)
            line_width = 3 if i == self.selected_region_idx else 2
            ...
            painter.drawText(scaled_x + 5, scaled_y + 20, label_type)
        ...
        if self.current_region:
            ...
            painter.setPen(pen)
            painter.drawRect(scaled_x, scaled_y, scaled_w, scaled_h)
            painter.drawText(scaled_x + 5, scaled_y + 20, "新区域")
...
class MainWindow(QMainWindow):
    ...
    def save_regions(self):
        """保存标记区域到文件"""
        yolo_regions = self.video_player.save_regions()
        if not yolo_regions:
            QMessageBox.warning(self, "警告", "没有可保存的标记区域")
            return

        # 保存为JSON文件
        save_path, _ = QFileDialog.getSaveFileName(
            self, "保存标注文件", "", "JSON文件 (*.json)"
        )

        if save_path:
            try:
                with open(save_path, 'w') as f:
                    json.dump(yolo_regions, f, indent=2)
                self.statusBar().showMessage(f"已保存标注到: {save_path}")
                QMessageBox.information(self, "成功", f"标注文件已保存到:\n{save_path}")
            except Exception as e:
                QMessageBox.critical(self, "错误", f"保存标注时出错:\n{str(e)}")
