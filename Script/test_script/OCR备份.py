...
class OCRProcessor(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # 1. 分析H.264码流
            self.log_message.emit("[H.264分析] 开始H.264码流分析...", self.LOG_ANALYSIS)
            self.analyze_h264_stream()

            # 2. 检查并修复视频
            if not self.check_and_repair_video():
                self.log_message.emit("[错误] 视频修复失败，无法继续处理", self.LOG_ERROR)
                return

            # 3. 打开视频
            cap = self.open_video_with_best_backend()
            if not cap or not cap.isOpened():
                raise Exception(f"[严重错误] 无法打开视频文件: {self.video_path}")

            # 获取视频信息
            fps = cap.get(cv2.CAP_PROP_FPS)
            total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            self.log_message.emit(f"[视频信息] 总帧数: {total_frames}, FPS: {fps:.2f}", self.LOG_INFO)
            ...
            last_valid_frame_count = 0  # 记录最后一个有效帧的编号
            ...
            while cap.isOpened() and not self.cancelled and frame_count < max_frame_attempts:
                try:
                    ...
                    if frame_count >= total_frames:
                        self.log_message.emit(f"[完成] 已处理所有帧 (共 {total_frames} 帧)", self.LOG_INFO)
                        break
                    ...
                    if not ret:
                        ...
                        if current_pos >= total_frames - 1:
                            self.log_message.emit(f"[完成] 已到达视频末尾 (帧 {frame_count}/{total_frames})",
                                                  self.LOG_INFO)
                            break
                        ...
                        if last_valid_frame is not None:
                            ...
                            self.log_message.emit(
                                f"[恢复] 使用上一有效帧替代损坏帧 {frame_count} (源帧 {last_valid_frame_count})",
                                self.LOG_RECOVERY
                            )
                        else:
                            # 没有有效帧可恢复，跳过
                    ...
                    for region in self.regions:
                        ...
                        try:
                            ...
                            self.log_message.emit(
                                f"[OCR错误] 帧 {frame_count} OCR失败: {str(e)}",
                                self.LOG_ERROR
                            )
                            detected_text = ""
                        ...
                        if ("你使用" in detected_text) and \
                            ...
                            if next_frame < total_frames:
                                ...
                                self.log_message.emit(
                                    f"[跳过] 检测到关键词，跳过 {skip_count} 帧至 {next_frame}",
                                    self.LOG_INFO
                                )
                            else:
                    ...
                    self.log_message.emit(
                        f"[{error_type}] 帧 {frame_count}: {error_msg}",
                        self.LOG_ERROR
                    )
                    ...
                    try:
                        ...
                        if next_frame >= total_frames:
                            self.log_message.emit(
                                f"[恢复] 已到达视频末尾，终止处理 (帧 {frame_count}/{total_frames})",
                                self.LOG_RECOVERY
                            )
                            break
                        ...
                        if next_frame < total_frames:
                            ...
                            frame_count = next_frame
                            self.log_message.emit(
                                f"[恢复] 跳过 {skip_after_error} 帧至 {next_frame}",
                                self.LOG_RECOVERY
                            )
                        else:
                            frame_count += 1
                    ...
                    if error_frame_count > max_error_frames:
                        self.log_message.emit(
                            f"[严重错误] 连续解码错误超过{max_error_frames}次，终止处理",
                            self.LOG_ERROR
                        )
                        break
                        ...
                        try:
                            ...
                            if cap.isOpened():
                                ...
                                self.log_message.emit(
                                    f"[恢复] 重新打开视频并定位到帧 {target_frame}/{total_frames}",
                                    self.LOG_RECOVERY
                                )
                                error_frame_count = 0  # 重置错误计数
                        except Exception as e:
                            self.log_message.emit(
                                f"[恢复失败] 无法重新打开视频: {str(e)}",
                                self.LOG_ERROR
                            )
            ...
            if frame_count >= total_frames:
                self.log_message.emit(
                    f"[完成] 已处理到视频末尾 (帧 {frame_count}/{total_frames})",
                    self.LOG_INFO
                )
            ...
            # 发送处理统计
            stats = f"处理完成: {processed_frames}帧处理, {skipped_frames}帧跳过"
...
class OCRProcessDialog(QDialog):
    ...
    def start_processing(self):
        ...
        self.ocr_processor = OCRProcessor(
            ...
            self.regions,
            self.keywords,
            padding=self.padding,  # 使用用户设置的预留时间
            skip_frames=3,
        ...
        # 新增：连接OCR文本检测信号
        self.ocr_processor.ocr_text_detected.connect(self.show_ocr_text)
        self.ocr_processor.log_message.connect(self.handle_log_message)
