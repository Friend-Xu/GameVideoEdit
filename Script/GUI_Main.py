...
class MainWindow(QMainWindow):
    ...
    def export_clips(self):
        """导出剪辑"""
        # 导入ExportDialog
        from FFmpeg_processor import ExportDialog

        # 获取视频路径（如果已打开）
        video_path = self.video_player.video_path

        # 这里简化处理，实际应用中应根据OCR结果生成剪辑时间段
        # 示例数据：每个剪辑片段是[start_time, end_time]的列表
        clip_data = None
        #     {
        #     "video_path": video_path,
        #     "clip_ranges": [
        #         [10.5, 15.2],  # 第一个剪辑片段：10.5秒到15.2秒
        #         [25.0, 30.5],  # 第二个剪辑片段：25秒到30.5秒
        #         [45.3, 50.8]  # 第三个剪辑片段：45.3秒到50.8秒
        #     ]
        # }

        # 如果打开了视频，传入视频路径和剪辑数据
        if video_path:
            self.export_dialog = ExportDialog(video_path, clip_data, self)
        else:
            # 没有打开视频，只传入剪辑数据
            self.export_dialog = ExportDialog(None, clip_data, self)

        if self.export_dialog.exec() == QDialog.Accepted:
...
if __name__ == "__main__":
    ...
    # 设置应用图标
    if os.path.exists("lighting.png"):
        app.setWindowIcon(QIcon("lighting.png"))
