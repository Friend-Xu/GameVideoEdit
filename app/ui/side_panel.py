...
class SidePanelWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        tbtns = QHBoxLayout()
        tbtns.addWidget(QPushButton("应用选中", clicked=self._apply_template_from_selection))
        tbtns.addWidget(QPushButton("保存当前", clicked=self._save_current_as_template))
        self._default_btn = QPushButton("设为默认")
        self._default_btn.clicked.connect(self._set_default_template)
        tbtns.addWidget(self._default_btn)
        tbtns.addWidget(QPushButton("删除", clicked=self._delete_template))
        gl2.addLayout(tbtns)
        layout.addWidget(g2)
